...
while(by_search <= int(behaivor_by_search)):
      if(opsy=="Windows"):
         ...
         cmd=('by_search python3 by_search.py ' + str(behaivor_margin_play) + ' ' + str(id_playlist) + ' ' +  str(t)+ ' by_search')
      subprocess.call(cmd, shell=True, cwd='scriptes/spotify/')
      print(cmd)
...
while(by_save <= int(behaivor_by_direct_save)):
      if(opsy=="Windows"):
         ...
         cmd=('by_search python3 by_save.py ' + str(behaivor_margin_play) + ' ' + str(id_playlist) + ' ' +  str(t) + ' by_search')
      subprocess.call(cmd, shell=True, cwd='scriptes/spotify/')
      print(cmd)
      by_save = by_save + 1
      sleep(10)  
...
while(by_album <= int(behaivor_by_album)):
      if(opsy=="Windows"):
         ...
         cmd=('by_search python3 by_album.py ' + str(behaivor_margin_play) + ' ' + str(id_playlist_album) + ' ' + str(t)  + ' by_search')
      subprocess.call(cmd, shell=True, cwd='scriptes/spotify/')
      print(cmd)
      by_album = by_album + 1
      sleep(10)  